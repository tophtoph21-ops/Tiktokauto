...
fs.mkdirSync(path.join(ROOT,'storage','audio'),{recursive:true});
fs.mkdirSync(path.join(ROOT,'storage','videos'),{recursive:true});
fs.mkdirSync(path.join(ROOT,'storage','images'),{recursive:true});
const secretFile=path.join(ROOT,'data','.app-secret');
if(!process.env.APP_SECRET){if(!fs.existsSync(secretFile))fs.writeFileSync(secretFile,crypto.randomBytes(32).toString('hex'));process.env.APP_SECRET=fs.readFileSync(secretFile,'utf8').trim()}
const APP_SECRET=process.env.APP_SECRET, PORT=Number(process.env.PORT||3000);
const db=new DatabaseSync(path.join(ROOT,'data','autopilot.db'));
db.exec(`PRAGMA journal_mode=WAL;
CREATE TABLE IF NOT EXISTS settings (k TEXT PRIMARY KEY, v TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS secrets (k TEXT PRIMARY KEY, v TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS products (id TEXT PRIMARY KEY, name TEXT NOT NULL, category TEXT, price REAL DEFAULT 0, commission_rate REAL DEFAULT 0, affiliate_url TEXT, notes TEXT, active INTEGER DEFAULT 1, created_at TEXT);
CREATE TABLE IF NOT EXISTS contents (id TEXT PRIMARY KEY, product_id TEXT, status TEXT NOT NULL, niche TEXT, format TEXT, title TEXT, hook TEXT, script TEXT, caption TEXT, cta TEXT, visual_beats TEXT, video_path TEXT, audio_path TEXT, scheduled_at TEXT, privacy_level TEXT DEFAULT 'SELF_ONLY', publish_id TEXT, tiktok_status TEXT, post_id TEXT, views INTEGER DEFAULT 0, clicks INTEGER DEFAULT 0, orders INTEGER DEFAULT 0, revenue REAL DEFAULT 0, error TEXT, created_at TEXT, updated_at TEXT);
CREATE TABLE IF NOT EXISTS oauth (provider TEXT PRIMARY KEY, open_id TEXT, access_token TEXT, refresh_token TEXT, expires_at INTEGER, refresh_expires_at INTEGER, scopes TEXT, updated_at TEXT);
CREATE TABLE IF NOT EXISTS oauth_state (state TEXT PRIMARY KEY, created_at INTEGER);
CREATE TABLE IF NOT EXISTS experiments (id TEXT PRIMARY KEY, product_id TEXT, winner_content_id TEXT, variant_count INTEGER DEFAULT 0, created_at TEXT);
CREATE TABLE IF NOT EXISTS events (id TEXT PRIMARY KEY, type TEXT, message TEXT, created_at TEXT);
`);
...
function log(type,message){try{db.prepare('INSERT INTO events(id,type,message,created_at) VALUES(?,?,?,?)').run(uid(),type,String(message),nowIso())}catch{}}
function staticFile(res,pathname){const aliases={'/terms':'terms.html','/privacy':'privacy.html'};const rel=aliases[pathname]||(pathname==='/'?'index.html':pathname.replace(/^\//,''));const file=path.join(ROOT,rel);if(!file.startsWith(ROOT)||!fs.existsSync(file)||fs.statSync(file).isDirectory())return false;const ext=path.extname(file),types={'.html':'text/html; charset=utf-8','.css':'text/css; charset=utf-8','.js':'text/javascript; charset=utf-8','.svg':'image/svg+xml','.png':'image/png','.jpg':'image/jpeg','.jpeg':'image/jpeg','.webmanifest':'application/manifest+json'};const b=fs.readFileSync(file);res.writeHead(200,{'content-type':types[ext]||'application/octet-stream','content-length':b.length});res.end(b);return true}
function serveStorage(res,pathname){const rel=pathname.replace(/^\/storage\//,'');const file=path.join(ROOT,'storage',rel);if(!file.startsWith(path.join(ROOT,'storage'))||!fs.existsSync(file))return false;const ext=path.extname(file).toLowerCase(),types={'.mp4':'video/mp4','.jpg':'image/jpeg','.jpeg':'image/jpeg','.png':'image/png','.mp3':'audio/mpeg','.m4a':'audio/mp4'};const b=fs.readFileSync(file);res.writeHead(200,{'content-type':types[ext]||'application/octet-stream','content-length':b.length,'accept-ranges':'bytes'});res.end(b);return true}
const product=id=>id?db.prepare('SELECT * FROM products WHERE id=?').get(id):null;
function publicPath(p){if(!p)return null;return '/storage/'+path.relative(path.join(ROOT,'storage'),p).split(path.sep).join('/')}
function content(id){const c=db.prepare('SELECT * FROM contents WHERE id=?').get(id);if(!c)return null;try{c.visual_beats=JSON.parse(c.visual_beats||'[]')}catch{c.visual_beats=[]}c.video_url=publicPath(c.video_path);c.image_url=publicPath(c.image_path);c.product=product(c.product_id);return c}
function saveOauth(x){const now=Date.now();db.prepare(`INSERT INTO oauth(provider,open_id,access_token,refresh_token,expires_at,refresh_expires_at,scopes,updated_at) VALUES('tiktok',?,?,?,?,?,?,?) ON CONFLICT(provider) DO UPDATE SET open_id=excluded.open_id,access_token=excluded.access_token,refresh_token=excluded.refresh_token,expires_at=excluded.expires_at,refresh_expires_at=excluded.refresh_expires_at,scopes=excluded.scopes,updated_at=excluded.updated_at`).run(x.open_id||'',encrypt(x.access_token,APP_SECRET),encrypt(x.refresh_token,APP_SECRET),now+(x.expires_in||86400)*1000,now+(x.refresh_expires_in||31536000)*1000,x.scope||'',nowIso())}
function getOauth(){const x=db.prepare('SELECT * FROM oauth WHERE provider=?').get('tiktok');return x?{...x,access_token:decrypt(x.access_token,APP_SECRET),refresh_token:decrypt(x.refresh_token,APP_SECRET)}:null}
async function accessToken(){let o=getOauth();if(!o)throw new Error('TikTok non connecté');if(o.expires_at-Date.now()<300000){const n=await tiktokRefresh(o.refresh_token);saveOauth(n);o=getOauth()}return o.access_token}
...
if(/^\/api\/contents\/[^/]+$/.test(p)&&req.method==='DELETE'){
  ...
  if(!c)return json(res,404,{error:'Vidéo introuvable'});
  for(const file of [c.video_path,c.audio_path,c.image_path]){
    if(file)try{if(fs.existsSync(file))fs.unlinkSync(file)}catch{}
  }
  db.prepare('DELETE FROM contents WHERE id=?').run(id);
  log('content_deleted',id);
  return json(res,200,{ok:true});
}
if(/^\/api\/contents\/[^/]+\/render$/.test(p)&&req.method==='POST')return json(res,200,{content:await renderOne(p.split('/')[3])});
if(/^\/api\/contents\/[^/]+\/approve$/.test(p)&&req.method==='POST'){const id=p.split('/')[3],b=await bodyJson(req),c=content(id);if(!c?.video_path)throw new Error('Crée la vidéo avant de la valider');db.prepare('UPDATE contents SET approved_at=?,scheduled_at=?,privacy_level=?,status=?,updated_at=? WHERE id=?').run(nowIso(),b.scheduled_at||nowIso(),b.privacy_level||'SELF_ONLY','SCHEDULED',nowIso(),id);return json(res,200,{content:content(id)})}
if(/^\/api\/contents\/[^/]+\/publish$/.test(p)&&req.method==='POST'){const id=p.split('/')[3],b=await bodyJson(req);if(!content(id)?.approved_at)db.prepare('UPDATE contents SET approved_at=?,privacy_level=?,updated_at=? WHERE id=?').run(nowIso(),b.privacy_level||'SELF_ONLY',nowIso(),id);return json(res,200,{content:await publishOne(id,b.privacy_level)})}
if(/^\/api\/contents\/[^/]+\/metrics$/.test(p)&&req.method==='POST'){const id=p.split('/')[3],b=await bodyJson(req);db.prepare('UPDATE contents SET views=?,clicks=?,orders=?,revenue=?,updated_at=? WHERE id=?').run(Number(b.views||0),Number(b.clicks||0),Number(b.orders||0),Number(b.revenue||0),nowIso(),id);recalcProductScores();return json(res,200,{content:content(id)})}
if(/^\/api\/contents\/[^/]+\/clone-winner$/.test(p)&&req.method==='POST'){const c=content(p.split('/')[3]);if(!c)throw new Error('Contenu introuvable');return json(res,201,{contents:await makeVariants(c.product_id,Number(getSetting('variants_per_product')))})}
if(p==='/api/tiktok/creator-info'&&req.method==='GET'){if(isDemo())return json(res,200,{creator_username:'demo',privacy_level_options:['SELF_ONLY','PUBLIC_TO_EVERYONE'],comment_disabled:false,duet_disabled:false,stitch_disabled:false,max_video_post_duration_sec:300});return json(res,200,await tiktokCreatorInfo(await accessToken()))}
if(p==='/api/sync-metrics'&&req.method==='POST')return json(res,200,await syncMetrics());
if(p==='/api/refresh-status'&&req.method==='POST'){await refreshStatuses();return json(res,200,{ok:true})}
if(p==='/api/optimize'&&req.method==='POST')return json(res,200,await optimize());
if(p==='/oauth/tiktok/start'&&req.method==='GET'){if(!process.env.TIKTOK_CLIENT_KEY||!process.env.TIKTOK_REDIRECT_URI)return json(res,400,{error:'Configure les identifiants TikTok dans Configuration'});const state=crypto.randomBytes(24).toString('base64url');db.prepare('INSERT INTO oauth_state(state,created_at) VALUES(?,?)').run(state,Date.now());const q=new URLSearchParams({client_key:process.env.TIKTOK_CLIENT_KEY,response_type:'code',scope:process.env.TIKTOK_SCOPES||'user.info.basic,video.publish,video.list',redirect_uri:process.env.TIKTOK_REDIRECT_URI,state});res.writeHead(302,{location:'https://www.tiktok.com/v2/auth/authorize/?'+q});return res.end()}
